...
object Comment extends SQLSyntaxSupport[Comment] {
  ...
  def apply(rs: WrappedResultSet): Comment = new Comment(
    rs.string(c.id),
    rs.string(c.user_id),
    rs.string(c.text),
    rs.string(c.parent_post_id),
    rs.int(c.comment_count),
    rs.date(c.posted_at)
  )
  ...
  //親Commentのコメント数を+1
  def updateCommentCount(comment_id: String = UUID.randomUUID.toString) =
    DB autoCommit { implicit session =>
